...
public class IncidentService {
    ...
    // My Custom Code
    /**
       * Get All Incidents for a Problem ID
       *
       * @param id - the id of the problem!
       * @param pageable the pagination information
       * @return a list of incidents caused by the given problem
       */
        @Transactional(readOnly = true)
        public Page<IncidentDTO> getRelatedIncidents(Pageable pageable, Long id) {
            log.debug("Incident Service: Get all Incidents Related to Problem: {}", id);
            Optional<Problem> tryProblem = problemRepository.findById(id);
            if (tryProblem.isPresent()) {
              Problem problem = tryProblem.get();
              return incidentRepository.findByProbRecOrderByOpenedAtDesc(pageable, problem).map(incidentMapper::toDto);
            } else {
              return null;
            }
        }
